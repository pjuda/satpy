...
from config import (MTGDIR, OUTDIR, RADIUS_OF_INFLUENCE, PATH_CTH_PAL)
from utils import (define_cth_colorbar, get_MTG_filenames, handle_missing_heights, ticks_meter_to_km,
                   lonlat_to_proj, initialize_mtg_df_in_target_proj, parallax_correct, time_of_interest_from_prompt)
...
cth_time_pattern =  '*CTXin' + datetime.strftime(start_datetime, '%Y%m%d%H%M') + '**.nc*'

cth_filename = glob(base_dir_cth + cth_time_pattern )
...
plt.savefig(out_dir + datetime.strftime(start_datetime, '%y%d%m_%H:%M-') + datetime.strftime(end_datetime, '%H:%M') + '_CMCTTH_LFL_NAN_' + domain_mtg, dpi = 300 )
print('Figure successfully saved at : ' + out_dir + datetime.strftime(start_datetime, '%y%d%m_%H:%M-') + datetime.strftime(end_datetime, '%H:%M') + '_CMCTTH_LFL_NAN_' + domain_mtg)
print('\n \n  *** Warnings satpy: *** \n')
plt.close(fig)
